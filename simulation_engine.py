...
from model_engine_v2 import minmax_score, model_engine_func
...
def aggregate_simulation_results(scenario_df: pd.DataFrame) -> pd.DataFrame:
    if scenario_df.empty:
        return pd.DataFrame()

    working = scenario_df.copy()
    working["is_top_10"] = (working["utility_rank"] <= 10).astype(int)
    working["scenario_score_for_agg"] = np.where(
        working["feasible_stop"] == 1,
        working["scenario_utility"],
        0.0,
    )

    summary = (
        working.groupby(["pin id", "pinname", "lat", "lng"], as_index=False)
        .agg(
...
def run_simulation(
    ...
    for run_id in range(1, int(num_runs) + 1):
        ...
        for hos_hour in HOS_HOURS:
            for time_name in ["morning", "afternoon", "evening"]:
                scenario_id = f"run_{run_id}_hos_{hos_hour}_{time_name}"
    ...
    summary_df = aggregate_simulation_results(scenario_df)
