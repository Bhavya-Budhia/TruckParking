...
def model_engine_func():
    ...
    def attach_last_obs_before_inference(query_df, obs_sorted, time_col="query_ts"):
        ...
        obs_sorted["ts_utc"] = pd.to_datetime(obs_sorted["ts_utc"], utc=True, errors="coerce").astype(
            "datetime64[ns, UTC]")
    ...
    truck_stop_df["HOS_exceeded"] = (
            truck_stop_df["time_to_stop_hr_adj"] > truck_stop_df["HOS_left_hr"]
    ).astype(int)

    truck_stop_df["detour_mi"] = (
            truck_stop_df["truck_stop_mi"] + truck_stop_df["stop_dest_mi"] - truck_stop_df["truck_dest_mi"]
    )
    ...
    truck_stop_df["utility_score"] = np.where(
        truck_stop_df["feasible_stop"] == 1,
        truck_stop_df["utility_score"],
        -999
    )

    truck_stop_df = truck_stop_df.sort_values("utility_score", ascending=False)
